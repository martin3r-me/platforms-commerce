<?php

namespace Platform\Commerce\Tools;

use Platform\Core\Contracts\ToolContract;
use Platform\Core\Contracts\ToolContext;
use Platform\Core\Contracts\ToolMetadataContract;
use Platform\Core\Contracts\ToolResult;
use Platform\Core\Models\Team;
use Platform\Core\Tools\Concerns\HasStandardizedWriteOperations;
use Platform\Commerce\Models\CommerceArticleAvailability;

/**
 * Aktualisiert eine bestehende Artikel-Verfügbarkeit.
 */
class UpdateArticleAvailabilityTool implements ToolContract, ToolMetadataContract
{
    use HasStandardizedWriteOperations;

    public function getName(): string
    {
        return 'commerce.article_availabilities.PATCH';
    }

    public function getDescription(): string
    {
        return 'PATCH /commerce/article-availabilities/{id} - Aktualisiert eine Artikel-Verfügbarkeit. Nutze commerce.article_availabilities.GET um die ID zu finden.';
    }

    public function getSchema(): array
    {
        return $this->mergeWriteSchema([
            'properties' => [
                'team_id' => [
                    'type' => 'integer',
                    'description' => 'Optional: Team-ID. Default: Team aus Kontext.',
                ],
                'id' => [
                    'type' => 'integer',
                    'description' => 'ID der Artikel-Verfügbarkeit (ERFORDERLICH). Nutze commerce.article_availabilities.GET.',
                ],
                'commerce_article_id' => [
                    'type' => 'integer',
                    'description' => 'Optional: Neue Artikel-ID.',
                ],
                'commerce_sales_context_id' => [
                    'type' => 'integer',
                    'description' => 'Optional: Neue Sales-Context-ID.',
                ],
                'is_available' => [
                    'type' => 'boolean',
                    'description' => 'Optional: Ist der Artikel verfügbar?',
                ],
                'available_from' => [
                    'type' => 'string',
                    'description' => 'Optional: Verfügbar ab (ISO 8601 Datum, "" zum Leeren).',
                ],
                'available_until' => [
                    'type' => 'string',
                    'description' => 'Optional: Verfügbar bis (ISO 8601 Datum, "" zum Leeren).',
                ],
                'max_quantity' => [
                    'type' => 'number',
                    'description' => 'Optional: Maximale bestellbare Menge (null zum Leeren).',
                ],
            ],
            'required' => ['id'],
        ]);
    }

    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            $teamId = $arguments['team_id'] ?? $context->team?->id;
            if ($teamId === 0 || $teamId === '0') {
                $teamId = null;
            }
            if (!$teamId) {
                return ToolResult::error('MISSING_TEAM', 'Kein Team angegeben und kein Team im Kontext gefunden.');
            }

            $team = Team::find((int)$teamId);
            if (!$team) {
                return ToolResult::error('TEAM_NOT_FOUND', 'Team nicht gefunden.');
            }

            if (!$context->user) {
                return ToolResult::error('AUTH_ERROR', 'Kein User im Kontext gefunden.');
            }
            $userHasAccess = $context->user->teams()->where('teams.id', $team->id)->exists();
            if (!$userHasAccess) {
                return ToolResult::error('ACCESS_DENIED', 'Du hast keinen Zugriff auf dieses Team.');
            }

            $found = $this->validateAndFindModel(
                $arguments,
                $context,
                'id',
                CommerceArticleAvailability::class,
                'NOT_FOUND',
                'Artikel-Verfügbarkeit nicht gefunden.'
            );
            if ($found['error']) {
                return $found['error'];
            }

            /** @var CommerceArticleAvailability $availability */
            $availability = $found['model'];
            if ((int)$availability->team_id !== (int)$team->id) {
                return ToolResult::error('ACCESS_DENIED', 'Artikel-Verfügbarkeit gehört nicht zum angegebenen Team.');
            }

            $update = [];

            if (array_key_exists('commerce_article_id', $arguments)) {
                $update['commerce_article_id'] = (int)$arguments['commerce_article_id'];
            }

            if (array_key_exists('commerce_sales_context_id', $arguments)) {
                $update['commerce_sales_context_id'] = (int)$arguments['commerce_sales_context_id'];
            }

            if (array_key_exists('is_available', $arguments)) {
                $update['is_available'] = (bool)$arguments['is_available'];
            }

            if (array_key_exists('available_from', $arguments)) {
                $v = (string)($arguments['available_from'] ?? '');
                $update['available_from'] = $v === '' ? null : $v;
            }

            if (array_key_exists('available_until', $arguments)) {
                $v = (string)($arguments['available_until'] ?? '');
                $update['available_until'] = $v === '' ? null : $v;
            }

            if (array_key_exists('max_quantity', $arguments)) {
                $update['max_quantity'] = $arguments['max_quantity'] !== null ? (float)$arguments['max_quantity'] : null;
            }

            if (!empty($update)) {
                $availability->update($update);
            }
            $availability->refresh();

            return ToolResult::success([
                'id' => $availability->id,
                'commerce_article_id' => $availability->commerce_article_id,
                'commerce_sales_context_id' => $availability->commerce_sales_context_id,
                'is_available' => (bool)$availability->is_available,
                'available_from' => $availability->available_from?->toIso8601String(),
                'available_until' => $availability->available_until?->toIso8601String(),
                'max_quantity' => $availability->max_quantity,
                'team_id' => $availability->team_id,
                'message' => 'Artikel-Verfügbarkeit erfolgreich aktualisiert.',
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Aktualisieren der Artikel-Verfügbarkeit: ' . $e->getMessage());
        }
    }

    public function getMetadata(): array
    {
        return [
            'category' => 'action',
            'tags' => ['commerce', 'article_availabilities', 'update'],
            'read_only' => false,
            'requires_auth' => true,
            'requires_team' => true,
            'risk_level' => 'write',
            'idempotent' => true,
        ];
    }
}
