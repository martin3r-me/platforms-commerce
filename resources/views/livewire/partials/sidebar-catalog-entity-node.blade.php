{{-- Rekursiver Entity-Knoten für Catalog-Sidebar — analog Planner-Sidebar --}}
@props(['node', 'typeIcon' => null, 'depth' => 0])

<div wire:key="catalog-entity-{{ $node['entity_id'] }}"
     x-data="{ open: localStorage.getItem('commerce.entity.' + {{ $node['entity_id'] }}) === 'true' }"
     class="flex flex-col">
    {{-- Entity-Zeile --}}
    <button type="button"
            @click="open = !open; localStorage.setItem('commerce.entity.' + {{ $node['entity_id'] }}, open)"
        ...
        <span class="ml-auto text-[10px] tabular-nums text-[var(--ui-muted)] opacity-60">{{ $node['total_catalogs'] }}</span>
    </button>

    {{-- Aufgeklappter Inhalt --}}
    <div x-show="open" x-collapse class="flex flex-col ml-3 border-l border-[var(--ui-border)]">
        {{-- 1. Eigene Kataloge --}}
        @foreach($node['catalogs'] as $catalog)
            <a wire:key="catalog-entity-{{ $node['entity_id'] }}-catalog-{{ $catalog->id }}"
               ...
               class="flex items-center gap-1.5 py-0.5 pl-3 pr-2 text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition truncate">
                ...
                @if(($catalog->status?->value ?? $catalog->status) === 'active')
                    <span class="w-1.5 h-1.5 rounded-full flex-shrink-0 ml-auto bg-green-500" title="Aktiv"></span>
                @elseif(($catalog->status?->value ?? $catalog->status) === 'archived')
                    <span class="w-1.5 h-1.5 rounded-full flex-shrink-0 ml-auto bg-gray-400 opacity-60" title="Archiviert"></span>
                @else
                    <span class="w-1.5 h-1.5 rounded-full flex-shrink-0 ml-auto bg-amber-400" title="Entwurf"></span>
                @endif
            </a>
        @endforeach
        ...
        @foreach($node['children_by_type'] as $typeGroup)
            <div wire:key="catalog-entity-{{ $node['entity_id'] }}-type-{{ $typeGroup['type_id'] }}"
                 x-data="{ groupOpen: localStorage.getItem('commerce.entity.' + {{ $node['entity_id'] }} + '.type.' + {{ $typeGroup['type_id'] }}) !== 'false' }"
                 class="flex flex-col">
                @if($node['children_by_type']->count() > 1 || $node['catalogs']->isNotEmpty())
                    <button type="button"
                            @click="groupOpen = !groupOpen; localStorage.setItem('commerce.entity.' + {{ $node['entity_id'] }} + '.type.' + {{ $typeGroup['type_id'] }}, groupOpen)"
